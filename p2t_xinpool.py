...
from cbam_module import CBAMBlock
from local_path import Spatial
...
__all__ = [
    'p2t_tiny', 'p2t_small', 'p2t_base', 'p2t_large', 'PatchEmbed'
]
...
class PoolingAttention(nn.Module):
    def __init__(self, dim, num_heads=2, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.,
        ...
        self.MHAttn = MHAttn(num_heads)  # 多头注意力
    ...
    def forward(self, x, H, W, d_convs=None):
        ...
        output_size = np.array([round(H / 2), round(W / 2)])
        stride = np.floor(input_size / output_size)
        kernel_size = input_size - (output_size-1) * stride
        stride = tuple(map(int, stride))
        kernel_size = tuple(map(int, kernel_size))
        pool = F.avg_pool2d(x_, kernel_size=kernel_size, stride=stride, padding=0)
        # pool = F.max_pool2d(x_, kernel_size=kernel_size, stride=stride, padding=0)
        pools = self.up(pool)
        ...
        # pools.append(pool1.view(B, C, -1))
        # pools.append(pool.view(B, C, -1))
        # pools.append(pool1.view(B, C, -1))
        # pools = [pool.view(B, C, -1), pool1.view(B, C, -1), pool.view(B, C, -1), pool1.view(B, C, -1)]

        # pools = torch.cat(pools, dim=2)
        pools = self.norm(pools.view(B, C, -1).permute(0, 2, 1))
...
class PyramidPoolingTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000, embed_dims=[64, 128, 320, 512],
        ...
        self.fusion = CBAMBlock(channel=704, reduction=16, kernel_size=7)

        self.conv = nn.Sequential(
            nn.Conv2d(704, 704, kernel_size=1, stride=1, padding=0),
            nn.BatchNorm2d(704),
            nn.ReLU(inplace=True),
        )

        self.norm_1 = nn.LayerNorm(704, eps=1e-6)
        self.classifier1 = nn.Sequential(
            nn.Linear(704, num_classes),
        )

        self.RepLK1 = RepLKBlock(31, 3, 3, prob=0.3)
        self.ConvFFN1 = ConvFFN(3, 3, prob=0.3)
        self.RepLK2 = RepLKBlock(29, 64, 64, prob=0.3)
        self.ConvFFN2 = ConvFFN(64, 64, prob=0.3)
        self.RepLK3 = RepLKBlock(15, 128, 128, prob=0.3)
        self.ConvFFN3 = ConvFFN(128, 128, prob=0.3)
        self.RepLK4 = RepLKBlock(9, 320, 320, prob=0.3)
        self.ConvFFN4 = ConvFFN(320, 320, prob=0.3)
    ...
    def forward_features(self, x):
        ...
        # x = self.SAttn4(x)
        x = x.reshape(B, 7, -1, 640).permute(0, 3, 1, 2)
        x = self.up_1(x)
    ...
    def forward(self, x):
        ...
        x = self.fusion(x)
        x = self.conv(x)
        x = nn.functional.avg_pool2d(x, kernel_size=[14, 14], stride=[14, 14], padding=0).reshape(x.shape[0], -1)
        x = self.norm_1(x)
